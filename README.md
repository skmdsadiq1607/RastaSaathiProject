 ...
 *   **Resilient Fallbacks**: Integrated mock fallback layers step in automatically to provide reliable routing if Google Places or database clusters encounter network lag.
 ...
 ### 🧠 3. Interactive AI Medical Assistant
 ...
 *   **Localization Support**: Communicates natively in the language selected in the user's profile (English, Hindi, Telugu, Tamil, Urdu).
 
 ### 📸 4. AI Injury Scanner (Vision Integration)
 Enables users to upload or capture photos of their road injuries for instant visual analysis.
 *   **Vision Diagnostics**: Leverages vision AI to recognize the injury type (e.g., cuts, burns, fractures) from the uploaded photo.
 *   **Instant Triage Guidance**: Immediately provides first aid steps, critical warnings (what NOT to do), and indicators of when to escalate to emergency care.
 
 ### 📄 5. High-Fidelity PDF Incident Reports
 ...
 *   **Conversational Transcript Logs**: Prints the complete, chronological chat history of the user-to-AI-Medic dialogue inside the document to serve as an audit-ready receipt for medical or legal verification.
 ...
 ### 💬 6. Multi-Channel Contact Alerts
 ...
 *   **Live Route Syncing**: Messages automatically bundle the victim's name, custom notes, live coordinates, and a one-click Google Maps driving link directing contacts from their location to the reserved hospital.
 ...
 Engineered to perform under real-world stress conditions, typically occurring on mobile devices on the road.
 *   **Fluid Layouts**: Grids and flex containers dynamically scale and stack on mobile devices.
 *   **Fluid Typography**: Uses CSS `clamp()` sizing to scale headers and buttons gracefully from 320px screens up to large monitors.
